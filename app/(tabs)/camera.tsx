...
export default function CameraScreen() {
  ...
  const frameProcessor = useSkiaFrameProcessor((frame) => {
    ...
    result.forEach((res) => {
      ...
      frame.drawRect(
        rect,
        getPaint({ style: 1, strokeWidth: 10, color: Skia.Color('red') }),
      );
      ...
      if (metrics.bounds) {
        frame.drawRect(
          Skia.XYWHRect(
            res.frame.x,
            res.frame.y - metrics?.bounds?.height,
            textWidth,
            metrics?.bounds?.height,
          ),
          getPaint({ color: Skia.Color('black') }),
        );
      }

      frame.drawText(
        `${label.text} ${(label.confidence * 100).toFixed(2)}%`,
        res.frame.x,
        res.frame.y,
        getPaint({ color: Skia.Color('white') }),
        font,
      );
    });
  }, []);
