                ...
                <View style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between' }}>
                    <View style={{ display: 'flex' }}>
                        <View>
                            ...
                            {data.abilities.map(({ ability }) => <ThemedText key={ability.name}>{removeHyphens(capitalize(ability.name))}</ThemedText>)}
                        </View>
                        <View style={{ marginTop: 16 }}>
                            <ThemedText style={{ textAlign: 'left', fontWeight: 'bold' }}>
                                Statistics:
                            </ThemedText>
                            {data.stats.map(({ stat, base_stat }) => <ThemedText key={stat.name} style={{ textAlign: 'left' }}>{removeHyphens(capitalize(stat.name))}: {base_stat}</ThemedText>)}
                        </View>
                    </View>
                    <View>
